...
def _scan_downloads(downloads_dir: Path, store: TriageDecisionStore) -> list[dict[str, Any]]:
    pdf_paths = sorted(
        (
            path
...
def create_app(downloads_dir: Path, staging_dir: Path, db_path: Path) -> Flask:
    ...
    app.config["triage_store"] = store

    @app.get("/")
    def review() -> str:
        decision_filter = request.args.get("filter", "all")
        files = _scan_downloads(downloads_dir, store)
